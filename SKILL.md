...
`/Users/martin_ai/Desktop/Martin Morning Brief/`

> **同步规则**：本文件是 skill 的权威副本（canonical copy）。对 skill 的任何修改都应更新此文件，然后同步到 `~/.claude/skills/morning-brief/SKILL.md`。
> ```bash
> cp "/Users/martin_ai/Desktop/Martin Morning Brief/SKILL.md" ~/.claude/skills/morning-brief/SKILL.md
> ```
...
如用户同意：
```bash
open "/Users/martin_ai/Desktop/Martin Morning Brief/output/interactive_dashboard.html"
```
...
| 数据库不存在 | 首次运行 | `run_daily.py` 会自动调用 `import_seed.py` |
