...
<!--                                                                      -->
<!-- ⚠️ 更新原则（重要）：                                               -->
<!--   ⚠️ 本适配版已移除市场数据抽取（看世界聚焦纯新闻，市场数据由主    -->
<!--      看板负责），不再抽取 15 个 Wind 指标，每日节省约 17 积分。     -->
<!--   ✅ 跟随更新：Step 0B (新闻搜索/筛选)、Step 0D (质检清单)          -->
<!--   ❌ 保持不动：Step 0C (HTML 模板/CSS 样式) — 网页设计使用交互式     -->
<!--                看板的统一规范，不随 Nina 更新                        -->
<!--                                                                      -->
<!-- 更新方法：当 Nina 仓库更新后，将新版 SKILL.md 仅替换 Step 0B、0D    -->
<!-- 两部分（Nina 的 Step 1 市场数据部分本适配版不再使用）。             -->
<!-- Wind MCP CLI 路径需适配本地环境（macOS / Windows）。                 -->
<!--                                                                      -->
...
Generate a polished, self-contained HTML report with bilingual news cards.
...
- [ ] 不抽取 Wind 市场数据（看世界聚焦新闻；市场数据由主看板负责）
...
运行 `@global-news-report` skill，或手动执行 Step 0B–0D。
